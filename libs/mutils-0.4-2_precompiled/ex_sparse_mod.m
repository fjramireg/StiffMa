...
% MESH = mtriangle(opts, tristr); % Is not working with my PC W10

nel = 50;                  % Number of elements at each direction
...
disp(['number of nodes:        ' num2str(nnod)]);
disp(['number of elements:     ' num2str(nel)]);
disp(['number of DOFs per node:' num2str(ndof)]);
...
t = tic;
As = sparse_create(MESH.ELEMS, Aelem_s, opts);   
disp(['1 CPU,  assemble symmetric sparse matrix (3 dof per node): ' num2str(toc(t))]);

% Parallel assembly of symmetric sparse matrix is executed by setting the
% desired number of threads in opts structure based on the renumbered mesh.
N = maxNumCompThreads;
for nt=1:N
    opts.nthreads = nt;  % parallel execution
    t = tic;
    As_par = sparse_create(MESHp.ELEMS, Aelem_s, opts);
    fprintf('%u CPUs, assemble renumbered symmetric sparse matrix (%u dof per node): %f \n', nt, ndof, toc(t));
...
t = tic;
ELEM_DOF = zeros(nelemdof, nel, 'uint32');
for dof=1:ndof
