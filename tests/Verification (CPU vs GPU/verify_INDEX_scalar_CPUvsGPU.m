...
[iKh, jKh] = Index_sssa(Mesh.elements, sets);               % indices for tril(K)

% MATLAB Computation on serial CPU (host. Optimized version)
% [iKh, jKh] = Index_sssa(Mesh.elements, sets);               % indices for tril(K)

% MATLAB Computation on parallel GPU (device. Old version)
[iKd, jKd] = Index_spsa(gpuArray(Mesh.elements'), sets);     % indices for tril(K)
