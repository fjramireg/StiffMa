...
t_opts mex2opts(const mxArray *opts_struct){
  ...
  /* inplace */
  field = mxGetField(opts_struct, 0, "inplace");
  opts.inplace = mex_get_integer_scalar(dimType, field, "inplace", 1, opts.inplace);
  ...
  VERBOSE("opts.inplace=%"PRI_DIMTYPE, DEBUG_BASIC, opts.inplace);
