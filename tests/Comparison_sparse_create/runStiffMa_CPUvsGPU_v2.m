function fullTable = runStiffMa_CPUvsGPU_v2
% Runs ALL code by varying problem size on vector/scalar problem for
...
nel_all  = [10:10:200];         %#ok Cases for mesh size. Limited by GPU memory
sets.sf  = 1;                   % Safety factor. Positive integer to add more partitions
prob_all = {'Scalar'};          % Cases for problem type
...
mkdir 'StiffMa_CPUvsGPU_scalar_v2';
addpath(genpath(pwd));
addpath(genpath('../../libs/mutils-0.4-2'));% sparse_create
addpath(genpath('../../tbx'));              % StiffMa

%% Move to results folder
cd 'StiffMa_CPUvsGPU_scalar_v2';
...
for k = 1:length(nel_all)
    ...
    for i = 1:length(proc_type)
        ...
        for pbl = 1:length(prob_all)
            ...
            % Prepares the test
            sets.name = ['Testing_',sets.proc_type,sets.prob_type,num2str(sets.nel)];
            WriteStiffMaPerfScriptCPUvsGPU_v2(sets);
