...
Ke_h = eStiff_sssa(Mesh, c, sets); % Element stiffness matrix computation - All tril(Ke)
...
if any(strcmp({v.Name}, 'Parallel Computing Toolbox'))
    ...
    Ke_d = eStiff_spsa(elementsGPU', nodesGPU', c, sets);   % tril(Ke) for all elements

    % Element matrix computation. Optimized version
    Ke_dopt = eStiff_spsa_opt(elementsGPU, nodesGPU, c, sets);    % tril(Ke) for all elements
