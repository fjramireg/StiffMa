function Plot_Assembly_runtimeMaxMeshSize
...
%% Data
load("Assembly_PerfTest_LNXMax2026.mat"); %#ok
...
h = plot(ax, ...
    nels, fullTable.Mean(1:stride:end),'-ob',...  % GPU_Scalar_single
    nels, fullTable.Mean(2:stride:end),'-+b',...  % GPU_Vector_single
    nels, fullTable.Mean(3:stride:end),'-or',...  % GPU_Scalar_double
    nels, fullTable.Mean(4:stride:end),'-+r');    % GPU_Vector_double
...
% Create legend
lg = legend(ax,{'GS_single','GV_single','GS_double','GV_double'});
% lg = legend(ax,...
%     {'CS32','GS32',...
%     'CV32','GV32',...
%     'CS64','GS64',...
