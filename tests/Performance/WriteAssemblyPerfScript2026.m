...
if strcmp(sets.prob_type,'Scalar')
    ...
    fprintf(fileID,'[iK, jK] = Index_spsa_opt(gpuArray(elements), sets);\n');
    fprintf(fileID,'Ke = eStiff_spsa_opt(gpuArray(elements), gpuArray(Mesh.nodes), c, sets);\n');
...
elseif strcmp(sets.prob_type,'Vector')
    ...
    fprintf(fileID,"sets.numSMs   = d.MultiprocessorCount;\n");
    fprintf(fileID,"sets.WarpSize = d.SIMDWidth;\n");   
    fprintf(fileID,'[iK, jK] = Index_vpsa_opt(gpuArray(elements), sets);\n');
    fprintf(fileID,'Ke = eStiff_vpsa_opt(gpuArray(elements), gpuArray(Mesh.nodes), MP, sets);\n');
    fprintf(fileID,'wait(d);\n');
