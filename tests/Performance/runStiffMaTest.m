function fullTable = runStiffMaTest
...
%   Created:  15/02/2020. Version: 1.4


%% Variables for performance tests
nel_all = [195:200];% Cases for mesh size. Limited by GPU memory
...
mkdir 'StiffMaPerfTestRst/';
cd 'StiffMaPerfTestRst/';
        ...
        for j = 1:length(dTNall)
            ...
            for pbl = 1:length(prob_all)
                ...
                for proc = 1:length(proc_all)
                    ...
                    % Prepares the test
                    sets.name = ['StiffMa',sets.pf,'_',sets.proc_type(1),sets.prob_type(1),...
                        sets.dTN(1),'u',sets.dTE(end-1:end),'_',num2str(sets.nel)];
                    ...
                    if ( ( strcmp(sets.proc_type,'CPU') && sets.nel > 20 ) ...
...
%% Save total results
fname = ['StiffMaPerfTest_',sets.pf];
