...
%   info = CPUINFO() returns a structure containing various bits of information
%   about the CPU and operating system as provided by /proc/cpu (Unix), sysctl
%   (Mac) or WMIC (Windows). This information includes: 
%     * CPU name
%     * CPU clock speed
%     * CPU Cache size (L2)
...
cleanup = onCleanup( @() fclose( fid ) );
% [status,txt]= system('lsb_release -a');
txt = textscan( fid, '%s', 'Delimiter', '\n' );
txt = txt{1};
