function K = StiffMa(elements,nodes,c,tbs)
% STIFFMA Create the global stiffness matrix for a SCALAR problem 
% taking advantage of symmetry and GPU computing. 
%   STIFFMA(elements,nodes,c,tbs) returns the lower-triangle of a sparse
%   matrix K from finite element analysis of scalar problems in a
...
%   nodal coordinates, "c" the material property for an isotropic material
%   and the optional "tbs" refers to ThreadBlockSize.  
%
...
% 	Modified: 05/12/2019. Version: 1.4. Variable number of inputs
...
if nargin == 3; tbs = 1024; end             % Default (MaxThreadsPerBlock)
...
%% Index computation
[iK, jK] = IndexScalarSymGPU(elements, tbs);    % Row/column indices of tril(K)

%% Element matrix computation
Ke = Hex8scalarSymGPU(elements,nodes,c, tbs); 	% Entries of tril(K)
