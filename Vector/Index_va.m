...
% ind_i2 = ind';
% ind_i3 = ind_i2(:);
% ind_i3 = reshape(ind', sets.nxe*sets.nel*sets.dxn,  1);
% ind_i3 = reshape([3*elements(:)-2, 3*elements(:)-1, 3*elements(:)]', sets.edof*sets.nel,  1);
ind = reshape([3*elements(:)-2, 3*elements(:)-1, 3*elements(:)]', sets.edof*sets.nel,  1);
% ind_i4 = reshape(ind_i3, sets.edof, sets.nel);
% ind_i4 = reshape([3*elements(:)-2, 3*elements(:)-1, 3*elements(:)]',  sets.edof, sets.nel);
% ind_i5 = repmat(reshape(ind_i3, sets.edof, sets.nel), sets.edof, 1);
% iK = ind_i5(:);
iK = reshape(repmat(reshape(ind, sets.edof, sets.nel), sets.edof, 1), sets.edof^2*sets.nel, 1);

% ind_j = repmat(ind, 1, sets.edof);
% % ind_j = repmat(ind_i4, 1, sets.edof);
% ind_j2 = ind_j';
% jK = ind_j2(:);

jK = reshape(repmat(ind, 1, sets.edof)', sets.edof^2*sets.nel, 1);
