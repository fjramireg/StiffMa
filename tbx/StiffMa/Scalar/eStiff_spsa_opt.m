function KE = eStiff_spsa_opt(elements, nodes, c, sets)
% ESTIFF_SPSA_OPT Computes the element stiffness matrices for a SCALAR (s)
% problem in PARALLEL (P) GPU computing taking advantage of symmetry (s)
% and returning ALL (a) ke for the mesh.
%   ESTIFF_SPSA_OPT(elements,nodes,c,sets) returns the element stiffness matrix
...
%   "nodes" the nodal coordinates of size Nx3, "c" (conductivity) is the
...
%   Created: July 8, 2026. Version: 1.0
...
if strcmp(sets.dTN,'single')        % Set constant memory on GPU
    setConstantMemory(ker, 'L_single',L);
elseif strcmp(sets.dTN,'double')
    setConstantMemory(ker, 'L_double',L);
end
% MATLAB KERNEL CALL
KE = feval(ker, elements, nodes, sets.nel, sets.nnodes, c, zeros(sets.sz*sets.nel, 1, sets.dTN, 'gpuArray'));
