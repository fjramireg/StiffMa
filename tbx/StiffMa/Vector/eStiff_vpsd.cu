...
*	Opt1:  nvcc -ptx eStiff_vpsd.cu
*   Opt2:  nvcc -ptx -v -arch=sm_50 --fmad=false -o eStiff_vpsd.ptx eStiff_vpsd.cu
*
** COMPILATION Within MATLAB
* 	setenv('MW_NVCC_PATH','/usr/local/cuda/bin')
*   setenv('PATH',[getenv('PATH') ':/usr/local/cuda/bin'])
*   system('nvcc -ptx eStiff_vpsd.cu')
*
...
__global__ void Hex8vector(const unsigned int *elements, const double *nodes, double *ke ) {
    // CUDA kernel to compute the NNZ entries or all tril(ke) (VECTOR)

    unsigned int e, i, j, k, l, m, temp, ni;                        // General indices
    double detJ, BDB, DB;                                           // Temporal scalars
    double x[8], y[8], z[8], invJ[9], B[6*24], dNdxyz[3*8];         // Temporal matrices
    ...
    for (e = threadIdx.x + blockDim.x * blockIdx.x; e < nel; e += gridDim.x * blockDim.x){
        ...
        for (i=0; i<8; i++) {                                       // Numerical integration (8 Gauss integration points)
            ...
            double J[9] = {0, 0, 0, 0, 0, 0, 0, 0, 0};              // Jacobian matrix
            for (j=0; j<8; j++) {
