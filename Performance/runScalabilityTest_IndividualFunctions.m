...
dTEall   = {'uint32','double'};                         % Cases for "element" data type
...
for k = 1:length(nel_all)
    ...
    for i = 1:length(dTEall)
        ...
        for j = 1:length(dTNall)
            dTN = dTNall{j};
            if (strcmp(dTN,'single') && strcmp(dTE,'double'))   % ommit this case
                continue;
            end
            ...
            for pt = 1:length(proc_all)
                ...
                for pbl = 1:length(prob_all)
                    ...
                    name = [pf,prob_type(1),'_',proctype,'_N',dTN,'_E',dTE,'_nel',num2str(nel),'_v2'];
