function fullTable = runAssemblyTest
...
nel_all = [95:100];% Cases for mesh size
...
mkdir 'AssemblyPerfTestRst/';
cd 'AssemblyPerfTestRst/';
        ...
        for j = 1:length(dTNall)
            ...
            for pbl = 1:length(prob_all)
                ...
                for proc = 1:length(proc_all)
                    ...
                    % Prepares the test
                    sets.name = ['ATest',sets.pf,'_',sets.proc_type(1),'_',sets.prob_type(1),...
                        sets.dTN(1),sets.dTE(end-1:end),'_',num2str(sets.nel)];
                    WriteAssemblyPerfScript(sets);
...
%% Save total results
fname = ['AssemblyPerfTest_',sets.pf];
