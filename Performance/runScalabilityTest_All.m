% Runs all code parts to assembly the stiffness matrix testing all
% possibilities and measuring their performance by using the built-in
% function 'runperf'
            ...
            for j = 1:length(dTNall)
                ...
                for pbl = 1:length(prob_all)
                    ...
                    elseif strcmp(codetype, 'GPU')
                        if strcmp(dTN,'single')
                            safetyFactor = 1;
                        elseif strcmp(dTN,'double')
                            safetyFactor = 1.5;
                            if strcmp(prob_type,'Vector')
                                safetyFactor = 2.5;
                            end
                        end
                    end
