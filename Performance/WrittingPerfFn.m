...
if strcmp(prob_type,'Scalar')
    ...
    elseif strcmp(proctype,'CPUp')
        ...
        fprintf(fileID,'c = 1;\n');
        fprintf(fileID,'N = size(nodes,1);\n');
        fprintf(fileID,'[iK, jK] = IndexScalarSymCPUp(elements);\n');
        fprintf(fileID,'Ke = Hex8scalarSymCPUp(elements,nodes,c);\n\n');
        fprintf(fileID,'%s\n','%% Index computation on CPU (parallel)');
        fprintf(fileID,'[i, j] = IndexScalarSymCPUp(elements);\n\n');
        fprintf(fileID,'%s\n','%% Element stiffness matrices computation on CPU (parallel)');
        fprintf(fileID,'v = Hex8scalarSymCPUp(elements,nodes,c);\n\n');
        if strcmp(dTN,'double')
            fprintf(fileID,'%s\n','%% Assembly of global sparse matrix on CPU');
            fprintf(fileID,'K = AssemblyStiffMat(iK,jK,Ke,N,dTE,dTN);\n');
        end
...
elseif strcmp(prob_type,'Vector')
    ...
    elseif strcmp(proctype,'CPUp')
        ...
        fprintf(fileID,'nu = 0.3;\n');
        fprintf(fileID,'N = size(nodes,1);\n');
        fprintf(fileID,'[iK, jK] = IndexVectorSymCPUp(elements);\n');
        fprintf(fileID,'Ke = Hex8vectorSymCPUp(elements,nodes,E,nu);\n\n');
        fprintf(fileID,'%s\n','%% Index computation on CPU (parallel)');
        fprintf(fileID,'[i, j] = IndexVectorSymCPUp(elements);\n\n');
        fprintf(fileID,'%s\n','%% Element stiffness matrices computation on CPU (parallel)');
        fprintf(fileID,'v = Hex8vectorSymCPUp(elements,nodes,E,nu);\n\n');
        if strcmp(dTN,'double')
            fprintf(fileID,'%s\n','%% Assembly of global sparse matrix on CPU');
            fprintf(fileID,'K = AssemblyStiffMat(iK,jK,Ke,3*N,dTE,dTN);\n');
        end
